...
if("treatment" %in% colnames(tasks_df)){
  ...
  tasks_df$treatment <- ifelse(tasks_df$treatment == 0, "No Symbionts", tasks_df$treatment)

  tasks_df$treatment <- ifelse(tasks_df$treatment == 1, "Mutualists", tasks_df$treatment)
}
...
molten_per_tasks <- subset(molten_tasks, update %% 2000 != 0)

small_per_count <- subset(counts_df, update %% 2000 != 0)
...
seed_plotter <- function(task, type){
  ...
  ggplot(subset(seeds_df, wanted_task == T & treatment == host_type), aes(x=update, y=value)) +
    ...
    seed_visuals + ylab(paste(host_type,task,"Task Counts")) + xlab("Updates")
...
#Instead of geom_line, must be geom_boxplot???

#facet wrapping by task, having nine plots with four boxes

#free y axis

box_df <- subset(molten_tasks, update == max(update) & host_task == TRUE)
