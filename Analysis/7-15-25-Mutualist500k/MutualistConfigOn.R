...
#Read in the data
data_1 <- read.table("munged_basic.dat", h=T)
data_2 <- read.table("munged_basic2.dat", h=T)
initial_data <- rbind(data_1, data_2)

initial_data$task <- factor(initial_data$task, levels=c("NOT", "NAND", "AND", "ORN", "OR", "ANDN", "NOR", "XOR", "EQU"))
initial_data[2] <- data.frame(lapply(initial_data[2], function(x) {gsub("MOI0.0", "MA", x)}))
initial_data[2] <- data.frame(lapply(initial_data[2], function(x) {gsub("MOI1.0", "MP", x)}))
...
#ggplot Nand by rep
nand_data <- subset(initial_data, task == "NAND")
nand_data <- subset(nand_data, treatment == "MP")


ggplot(data=initial_data, aes(x=update, y=count, group=partner, colour=partner)) + ylab("Task count") + xlab("Updates") + stat_summary(aes(color=partner, fill=partner),fun.data="mean_cl_boot", geom=c("smooth")) + theme(panel.background = element_rect(fill='white', colour='black')) + theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank()) + guides(fill=FALSE) +scale_colour_manual(values=viridis(4)) + scale_fill_manual(values=viridis(4)) + facet_wrap(~rep, scales='free')
