...
mutualists_data <- read.table("../8-7-25-NutrientMutualistFullDonate/munged_basic.dat", h=T)
mutualists_data2<- read.table("../8-7-25-NutrientMutualistFullDonate/munged_basic2.dat", h=T)

combined_mutualists_data <- rbind(mutualists_data, mutualists_data2)
combined_mutualists_data[2] <- data.frame(lapply(combined_mutualists_data[2], function(x) {gsub("MOI0.0", "No Symbionts", x)}))
combined_mutualists_data[2] <- data.frame(lapply(combined_mutualists_data[2], function(x) {gsub("MOI1.0", "Mutualists", x)}))


parasites_data <- read.table("../8-5-25-NutrientParasitesFullSteal/munged_basic.dat", h=T)
parasites_data2 <- read.table("../8-5-25-NutrientParasitesFullSteal/munged_basic2.dat", h=T)

combined_parasites_data <- rbind(parasites_data, parasites_data2)
combined_parasites_data[2] <- data.frame(lapply(combined_parasites_data[2], function(x) {gsub("MOI0.0", "No Symbionts", x)}))
combined_parasites_data[2] <- data.frame(lapply(combined_parasites_data[2], function(x) {gsub("MOI1.0", "Parasites", x)}))  
parasites_only_sym <- subset(combined_parasites_data, treatment == "Parasites")

initial_data <- rbind(combined_mutualists_data, parasites_only_sym)
...
#Wilcox Tests: Mutualists VS No Symbionts
...
#Wilcox Tests: Parasites VS No Symbionts
