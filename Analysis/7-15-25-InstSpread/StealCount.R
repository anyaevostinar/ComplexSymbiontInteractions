...
initial_data[2] <- data.frame(lapply(initial_data[2], function(x) {gsub("AMOUNT25", "START 25", x)}))
initial_data[2] <- data.frame(lapply(initial_data[2], function(x) {gsub("AMOUNT35", "START 35", x)}))
initial_data[2] <- data.frame(lapply(initial_data[2], function(x) {gsub("AMOUNT50", "START 50", x)}))
...
#All tasks over time
ggplot(data=initial_data, aes(x=update, y=steal_count, group=treatment, colour=treatment)) + ylab("Task count") + xlab("Updates") + stat_summary(aes(color=treatment, fill=treatment),fun.data="mean_cl_boot", geom=c("smooth"), se=TRUE) + theme(panel.background = element_rect(fill='white', colour='black')) + theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank()) + guides(fill=FALSE) +scale_colour_manual(values=viridis(4)) + scale_fill_manual(values=viridis(4))
