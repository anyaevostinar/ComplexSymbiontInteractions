...
initial_data[2] <- data.frame(lapply(initial_data[2], function(x) {gsub("CPU_TRANSFER_CHANCE0.6", "60%", x)}))
initial_data[2] <- data.frame(lapply(initial_data[2], function(x) {gsub("CPU_TRANSFER_CHANCE0.7", "70%", x)}))
initial_data[2] <- data.frame(lapply(initial_data[2], function(x) {gsub("CPU_TRANSFER_CHANCE0.8", "80%", x)}))
initial_data[2] <- data.frame(lapply(initial_data[2], function(x) {gsub("CPU_TRANSFER_AMOUNT4", "4 Updates", x)}))
initial_data[2] <- data.frame(lapply(initial_data[2], function(x) {gsub("CPU_TRANSFER_CHANCE6", "6 Updates", x)}))
initial_data[2] <- data.frame(lapply(initial_data[2], function(x) {gsub("_", " ", x)}))
final_update <- subset(initial_data, update == "99999")
