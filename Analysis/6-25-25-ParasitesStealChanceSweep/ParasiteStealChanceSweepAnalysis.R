...
#Read in the data
initial_data <- read.table("6-25-25-ParasiteStealChanceSweep/munged_basic.dat", h=T)
initial_data$task <- factor(initial_data$task, levels=c("NOT", "NAND", "AND", "ORN", "OR", "ANDN", "NOR", "XOR", "EQU"))
initial_data[2] <- data.frame(lapply(initial_data[2], function(x) {gsub("CPU_CHANCE0.6", "60%", x)}))
initial_data[2] <- data.frame(lapply(initial_data[2], function(x) {gsub("CPU_CHANCE0.7", "70%", x)}))
initial_data[2] <- data.frame(lapply(initial_data[2], function(x) {gsub("CPU_CHANCE0.8", "80%", x)}))
initial_data[2] <- data.frame(lapply(initial_data[2], function(x) {gsub("CPU_CHANCE0.9", "90%", x)}))
