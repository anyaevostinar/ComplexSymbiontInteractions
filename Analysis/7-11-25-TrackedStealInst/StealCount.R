...
#Read in the data
initial_data <- read.table("steal_munged_basic.dat", h=T)


initial_data[2] <- data.frame(lapply(initial_data[2], function(x) {gsub("CHANCE0.8", "80%", x)}))
initial_data[2] <- data.frame(lapply(initial_data[2], function(x) {gsub("CHANCE0.9", "90%", x)}))

initial_data[2] <- data.frame(lapply(initial_data[2], function(x) {gsub("AMOUNT10", "START 10", x)}))
initial_data[2] <- data.frame(lapply(initial_data[2], function(x) {gsub("AMOUNT15", "START 15", x)}))
initial_data[2] <- data.frame(lapply(initial_data[2], function(x) {gsub("AMOUNT23", "START 23", x)}))

initial_data[2] <- data.frame(lapply(initial_data[2], function(x) {gsub("WAIT10", "BONUS 10", x)}))
initial_data[2] <- data.frame(lapply(initial_data[2], function(x) {gsub("WAIT20", "BONUS 20", x)}))
initial_data[2] <- data.frame(lapply(initial_data[2], function(x) {gsub("WAIT40", "BONUS 40", x)}))
...
host_data <- subset(final_update, treatment == "Parasite")
...
#All tasks over time
ggplot(data=initial_data, aes(x=update, y=stealcount, group=treatment, colour=treatment)) + ylab("Task count") + xlab("Updates") + stat_summary(aes(color=treatment, fill=treatment),fun.data="mean_cl_boot", geom=c("smooth"), se=TRUE) + theme(panel.background = element_rect(fill='white', colour='black')) + theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank()) + guides(fill=FALSE) +scale_colour_manual(values=viridis(18)) + scale_fill_manual(values=viridis(18))
