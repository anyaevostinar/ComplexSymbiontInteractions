...
for a in seeds:
    for b in start_MOI:
        for c in Horiz_sweep:
            command_str = './symbulation_sgp -SEED '+str(a)+ ' -START_MOI ' +str(b)+' -HOST_REPRO_RES ' +str(c)+ ' -FILE_NAME _MOI'+str(b)+'_REPRO'+str(c)
            settings_filename = "Output_MOI"+str(b)+'_REPRO'+str(c)+"_SEED"+str(a)+".data"
