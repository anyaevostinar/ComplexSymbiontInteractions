...
for a in seeds:
    for b in steal_chance:
        for c in steal_amount:
            command_str = './symbulation_sgp -SEED '+str(a)+ ' -CPU_TRANSFER_CHANCE ' +str(b)+ ' -CPU_TRANSFER_AMOUNT ' +str(c)+ ' -FILE_NAME _CPU_TRANSFER_CHANCE'+str(b)+'AMOUNT'+str(c)
            settings_filename = "Output_CPU_TRANSFER_CHANCE"+str(b)+'AMOUNT'+str(c)+"_SEED"+str(a)+".data"
